...
const { SubscriptionManager, CustomerManager } = require("../services/stripeConfig");
...
router.post(
  ...
  limiters.billing,
  authenticate,
  [body("priceId").notEmpty(), body("paymentMethodId").notEmpty()],
  handleValidationErrors,
  async (req, res, next) => {
    try {
 ...
 */
router.get("/subscription/:subscriptionId", authenticate, async (req, res, next) => {
  try {
 ...
 */
router.get("/invoices/:invoiceId/pdf", authenticate, async (req, res, next) => {
  try {

...
export class DriverAvailabilityPredictor {
  ...
  private calculateHistoricalFactor(history: DriverAvailabilityData[], time: Date): number {
    ...
    const similarRecords = history.filter((h) => h.timeOfDay === hour && h.dayOfWeek === dayOfWeek);
...
export async function predictDriverAvailability(req: Request, res: Response) {
  const { driverId, weatherCondition, trafficLevel, recentLoadCount, currentTime } = req.body ?? {};
 ...
 */
export async function getDispatchRecommendations(req: Request, res: Response): Promise<void> {
  try {
    const {
      targetTime,
      minAvailability = 0.5,
    ...
    const availabilityThreshold = Math.max(0, Math.min(1, Number(minAvailability) || 0));
    const maxResults = Math.max(1, Math.floor(Number(limit) || 0));

    const recommendations = predictions
      .filter((prediction) => prediction.availabilityProbability >= availabilityThreshold)

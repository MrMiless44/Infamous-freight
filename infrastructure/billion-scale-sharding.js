...
class BillionScaleShardingService {
  ...
  async readUser(userId) {
    ...
    try {
      ...
      const result = await replicaConn.query("SELECT * FROM users_replica WHERE user_id = $1", [
        userId,
      ]);
    ...
    // Fallback to primary
    const primaryConn = this.shardConnections.get(shard.primary);
    const result = await primaryConn.query("SELECT * FROM users WHERE id = $1", [userId]);
  ...
  async migrateUser(userId, newRing) {
    ...
    await newShardConn.query(`INSERT INTO users (id, data) VALUES ($1, $2)`, [
      userId,
...
class ConsistentHashRing {
  ...
  getNode(hashValue) {
    const hash =
      typeof hashValue === "string" ? parseInt(hashValue.substring(0, 8), 16) : hashValue;

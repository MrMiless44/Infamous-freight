...
echo "WARNING: Deploying API with TypeScript checks disabled (API_BUILD_NO_CHECK=true)."
echo "Deploying API app '$TARGET_APP' using $CONFIG_PATH"
exec "$FLYCTL_BIN" deploy -c "$CONFIG_PATH" --app "$TARGET_APP" --build-arg API_BUILD_NO_CHECK=true

...
const load = loads.find((l) => l.status === 'in_transit') ?? loads[0];

const checklistItems = [
  { id: 'intact', label: 'Cargo intact and undamaged' },
  { id: 'signed', label: 'Receiver signed delivery receipt' },
  { id: 'photos', label: 'Delivery photos taken' },
  { id: 'pod', label: 'Proof of Delivery (POD) uploaded' },
];
...
export default function DeliveryConfirmationPage() {
  ...
  const handlePhotoUpload = () => {
    setPhotos((prev) => [...prev, `delivery-photo-${prev.length + 1}.jpg`]);
  };
  ...
  if (confirmed) {
    return (
      <div className="mx-auto max-w-lg py-20 text-center">
        ...
        <h1 className="text-xl font-bold text-white">Delivery Confirmed</h1>
        <p className="mt-2 text-sm text-slate-400">Load {load.loadNumber} delivered successfully.</p>
        <Link
          href="/driver/loads"
  ...
  return (
    <div className="mx-auto max-w-lg space-y-5 pb-8">
      ...
      {/* Checklist */}
      <div className="rounded-xl border border-slate-800 bg-[#161b22] p-4">
        <h2 className="mb-3 text-sm font-semibold text-white">Delivery Checklist</h2>
      ...
      {/* Signature Area */}
      <div className="rounded-xl border border-slate-800 bg-[#161b22] p-4">
        <h2 className="mb-3 text-sm font-semibold text-white">Receiver Signature</h2>
        <div className="flex h-32 items-center justify-center rounded-lg border-2 border-dashed border-slate-700 bg-[#0d1117]">
          <p className="text-xs text-slate-500">Tap to capture signature</p>
        </div>
      </div>

      {/* Photo Upload */}
      <div className="rounded-xl border border-slate-800 bg-[#161b22] p-4">
        <h2 className="mb-3 text-sm font-semibold text-white">Proof of Delivery Photos</h2>
        <button
          type="button"

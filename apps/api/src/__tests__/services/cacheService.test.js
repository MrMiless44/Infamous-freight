...
describe("Redis Cache Service", () => {
  ...
  describe("Basic Operations", () => {
    ...
    describe("set", () => {
      ...
      it("should handle Redis errors", async () => {
        ...
        await expect(cacheService.set("error:key", { test: "data" })).resolves.not.toThrow();
      });
    });
    ...
    describe("invalidate", () => {
      it("should invalidate keys matching pattern", async () => {
        mockRedisClient.keysAsync.mockResolvedValue(["test:key:1", "test:key:2", "test:key:3"]);
        mockRedisClient.delAsync.mockResolvedValue(1);
      ...
      it("should handle Redis errors", async () => {
        ...
        await expect(cacheService.invalidate("error:*")).resolves.not.toThrow();
      });
    });
  });
  ...
  describe("Job Caching", () => {
    ...
    describe("getNearbyJobs", () => {
      it("should retrieve nearby jobs", async () => {
        ...
        const result = await cacheService.getNearbyJobs(40.7128, -74.006);
      ...
      it("should round coordinates", async () => {
        ...
        expect(mockRedisClient.getAsync).toHaveBeenCalledWith("jobs:nearby:40.71:-74.01:10");
      });

      it("should support custom radius", async () => {
        mockRedisClient.getAsync.mockResolvedValue(null);

        await cacheService.getNearbyJobs(40.7128, -74.006, 25);

        expect(mockRedisClient.getAsync).toHaveBeenCalledWith("jobs:nearby:40.71:-74.01:25");
      });
    });
    ...
    describe("setNearbyJobs", () => {
      it("should cache nearby jobs", async () => {
        ...
        await cacheService.setNearbyJobs(40.7128, -74.006, jobs);
  ...
  describe("TTL Configuration", () => {
    ...
    it("should use jobListTTL for nearby jobs", async () => {
      ...
      await cacheService.setNearbyJobs(40.7128, -74.006, []);
  ...
  describe("Error Resilience", () => {
    ...
    it("should not throw on cache write errors", async () => {
      ...
      await expect(cacheService.set("test:key", { test: "data" })).resolves.not.toThrow();
    });

    ...
    constructor() {
        ...
        this.extractionTemplates = {
            BOL: [
                ...
                'bolNumber', 'date', 'carrier', 'weight', 'pieces', 'description',
                'specialInstructions', 'freightCharges'
            ],
            POD: [
                'deliveryDate', 'deliveryTime', 'recipientName', 'recipientSignature',
                'bolNumber', 'condition', 'notes'
            ],
            INVOICE: [
                'invoiceNumber', 'invoiceDate', 'dueDate', 'billingAddress',
                'shipmentId', 'description', 'quantity', 'rate', 'amount',
                'subtotal', 'tax', 'total'
            ],
            RECEIPT: [
                'date', 'vendor', 'amount', 'category', 'paymentMethod', 'items'
            ],
            PERMIT: [
                'permitNumber', 'issueDate', 'expiryDate', 'jurisdiction',
                'vehicle', 'restrictions', 'fee'
            ]
        };
    }
    ...
    async batchProcessDocuments(documents) {
        try {
            ...
            const results = await Promise.all(
                documents.map(doc =>
                    this.processDocument(doc.buffer, doc.type, doc.options)
                        .catch(error => ({
                            error: error.message,
                            documentId: doc.id
                        }))

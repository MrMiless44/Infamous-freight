...
class PricingOptimizationService {
     ...
     */
    async calculateOptimalShipperRate(
        shipment,
        marketData = {},
        ...
        try {
            ...
            const shipperFactor = this.calculateShipperFactor(
                marketData.shipperRating || 4.0
            );
            ...
            for (let priceMultiplier = 0.8; priceMultiplier <= 1.2; priceMultiplier += 0.1) {
                ...
                conversionPredictions.push({
                    ...
                    expectedBids: Math.round(
                        (marketData.availableDrivers || 100) * conversionRate
                    ),
                });
            }
            ...
            const result = {
                ...
                optimalPrice: this.findOptimalPrice(
                    conversionPredictions,
                    recommendedRate
                ),
    ...
    async getCorridorRateTrends(corridor, days = 30) {
        try {
            ...
            const firstHalf = rates.slice(0, Math.floor(rates.length / 2)).reduce((a, b) => a + b) /
                Math.floor(rates.length / 2);
            const secondHalf = rates.slice(Math.floor(rates.length / 2)).reduce((a, b) => a + b) /
    ...
    async getCompetitorAnalysis(corridor, ourRate) {
        try {
            ...
            const competitorRates = [
                ...
                { competitor: "Schneider", rate: 1400, marketShare: 0.10 },
            ];
            ...
            const ourCompetitiveness = {
                ...
                percentDifference: Math.round(
                    ((ourRate - avgCompetitorRate) / avgCompetitorRate) * 100
                ),
                ranking:
            ...
            return {
                ...
                recommendation: this.getCompetitivenessRecommendation(
                    ourCompetitiveness
                ),
            };
        } catch (err) {
    ...
    generatePricingReasoning(marketData, shipment) {
        ...
        if (
            (new Date(shipment.pickupDate) - Date.now()) /
            (1000 * 60 * 60) <
            24
        ) {
    ...
    getCompetitivenessRecommendation(competitiveness) {
        if (competitiveness.percentDifference > 10)
            return "Reduce price to compete";
        if (competitiveness.percentDifference < -10)
            return "Increase price - below market";

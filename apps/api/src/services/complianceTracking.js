...
class ComplianceService {
    ...
    async checkHOSViolations(driverId) {
        try {
            ...
            const todayOnDuty = this.sumHours(todayLogs.filter(l => ['driving', 'on_duty'].includes(l.activityType)));
            ...
            const weeklyHours = this.sumHours(logs.filter(l => ['driving', 'on_duty'].includes(l.activityType)));
    ...
    async getDriverHOSStatus(driverId) {
        try {
            ...
            const onDutyHours = this.sumHours(todayLogs.filter(l => ['driving', 'on_duty'].includes(l.activityType)));
            const weeklyHours = this.sumHours(weeklyLogs.filter(l => ['driving', 'on_duty'].includes(l.activityType)));
        ...
        for (const log of logs) {
            if (log.activityType === 'driving') {
                ...
                if (drivingTime >= afterHours) {
                    ...
                    const breakLog = logs.find(l =>
                        l.activityType === 'off_duty' &&
                        new Date(l.startTime) >= new Date(log.endTime)
                    );

                    if (breakLog) {
                        const breakDuration = (new Date(breakLog.endTime) - new Date(breakLog.startTime)) / (1000 * 60 * 60);
    ...
    generateRecommendations(violations) {
        ...
        if (criticalCount > 5) {
            recommendations.push('High number of critical violations. Immediate review of driver scheduling required.');
        }

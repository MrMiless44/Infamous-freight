...
class RevenueMonitor {
  constructor(options = {}) {
    this.thresholds = {
      ...
      monthlyGrowth: options.monthlyGrowth || 0.1, // 10% month-over-month growth
  ...
  async checkDailyRevenue() {
    ...
    const lastWeekToday = await this.getRevenueForDate(new Date(Date.now() - 7 * 86400000));
  ...
  async checkChurnRate() {
    ...
    const activeAtMonthStart = await prisma.subscription.count({
      where: {
        ...
        OR: [{ status: "active" }, { status: "cancelled", cancelledAt: { gte: startOfMonth } }],
      },
    });

    const churnRate = activeAtMonthStart > 0 ? cancelledThisMonth / activeAtMonthStart : 0;
  ...
  async runDailyHealthCheck() {
    ...
    try {
      const [revenueCheck, weeklyGrowth, churnCheck, mrrTrend, paymentFailures] = await Promise.all(
        [
  ...
  async getMRRForMonth(monthsAgo = 0) {
    ...
    const result = await prisma.subscription.aggregate({
      where: {
        ...
        OR: [{ cancelledAt: null }, { cancelledAt: { gte: targetMonth } }],
      },
      _sum: { monthlyValue: true },
    });

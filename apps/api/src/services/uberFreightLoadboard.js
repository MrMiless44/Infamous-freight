...
class UberFreightLoadboard {
    ...
    async search(origin, destination, filters = {}) {
        try {
            ...
            const loads = response.data.shipments.map((shipment) =>
                this.normalizeLoad(shipment)
            );
    ...
    async getLoadDetail(loadId, externalId) {
        try {
            ...
            const response = await axios.get(
                `${this.apiBase}/shipments/${externalId}`,
                {
                    headers: {
                        Authorization: `Bearer ${this.accessToken}`,
                    },
    ...
    async getStats() {
        try {
            ...
            const response = await axios.get(
                `${this.apiBase}/shipments/available/stats`,
                {
                    headers: {
                        Authorization: `Bearer ${this.accessToken}`,
                    },
    ...
    calculateScore(shipment, miles) {
        ...
        if (shipment.equipment_type === "dry_van") {
            ...
            ["reefer", "flatbed", "tanker"].includes(shipment.equipment_type)
        ) {
            score += 5; // Specialty
        }

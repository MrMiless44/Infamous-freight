...
class AdvancedGeofencingService {
  ...
  createSafetyCorridor(corridor) {
    try {
      ...
      logger.info("Safety corridor created", {
        corridorId,
        name: corridor.name,
        distance: this.calculateDistance(corridor.startLocation, corridor.endLocation),
      });
  ...
  async updateDriverLocation(driverId, location) {
    try {
      ...
      const corridorEvents = await this.checkCorridorCompliance(driverId, currentLocation);

      // Check speeding violations
      const speedingViolations = await this.checkSpeedingViolations(driverId, currentLocation);
  ...
  async checkZoneBoundaries(driverId, previousLocation, currentLocation) {
    try {
      ...
      for (const [zoneId, zone] of this.zones) {
        ...
        const wasInZone = previousLocation ? this.isLocationInZone(previousLocation, zone) : false;
  ...
  async checkSpeedingViolations(driverId, location) {
    try {
      ...
      for (const [zoneId, zone] of this.zones) {
        if (!zone.active || !zone.speed_limit || !this.isLocationInZone(location, zone)) {
          continue;
        }
        ...
        if (location.speed > zone.speed_limit) {
          const event = {
            ...
            timestamp: location.timestamp,
            severity: this.getSpeedingSeverity(location.speed - zone.speed_limit),
          };
  ...
  distancePointToSegment(point, segStart, segEnd) {
    ...
    let t =
      ((point.lon - segStart.lon) * dx + (point.lat - segStart.lat) * dy) / (dx * dx + dy * dy);

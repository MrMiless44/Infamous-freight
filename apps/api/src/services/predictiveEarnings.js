...
class PredictiveEarningsService {
  ...
  async forecastEarnings(driverId, params = {}) {
    try {
      ...
      // Generate forecast
      const forecast = this.generateForecast(history, trends, marketFactors, days, granularity);
  ...
  generateForecast(history, trends, marketFactors, days, granularity) {
    ...
    forecast.summary = {
      ...
      percentageChange: (
        ((predictions[predictions.length - 1] - predictions[0]) / predictions[0]) *
        100
      ).toFixed(1),
    };
  ...
  calculateOverallConfidence(trends) {
    ...
    if (trends.volatility < 0.1) confidence += 0.1;
      ...
      if (period === "weekly") {
        for (let week = 1; week <= 12; week++) {
          ...
          milestones.push({
            ...
            confidence: (weekData.reduce((s, d) => s + d.confidence, 0) / weekData.length).toFixed(
              2,
            ),
          });
        }

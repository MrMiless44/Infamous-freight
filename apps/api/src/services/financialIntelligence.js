...
class FinancialIntelligenceService {
  constructor() {
    ...
    this.standardRates = {
      full_truckload: { min: 1.5, max: 3.5, avg: 2.25 }, // per mile
      less_than_truckload: { min: 0.8, max: 2.0, avg: 1.3 },
      expedited: { min: 3.0, max: 6.0, avg: 4.25 },
      hazmat: { min: 2.5, max: 5.0, avg: 3.5 },
    };

    // Standard cost factors
    this.costFactors = {
      fuel: { perMile: 0.7, perGallon: 4.5 },
      driver: { perMile: 0.45, hourly: 28.0 },
      maintenance: { perMile: 0.15 },
      insurance: { perMile: 0.12 },
      tolls: { perMile: 0.08 },
      overhead: { dailyFixed: 150.0 },
    };
  }
  ...
  async calculateShipmentPL(shipmentId) {
    try {
      ...
      pl.revenue = {
        ...
        total:
          (shipment.rate || 0) + (shipment.fuelSurcharge || 0) + (shipment.accessorialCharges || 0),
      };
      ...
      pl.metrics = {
        ...
        breakEvenMiles:
          pl.metrics?.revenuePerMile > pl.metrics?.costPerMile

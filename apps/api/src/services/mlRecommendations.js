...
class MLLoadRecommendations {
  constructor() {
    ...
    this.featureWeights = {
      ...
      driverHistory: 0.2, // Driver's success rate
      marketDemand: 0.15, // Loads available on corridor
      timeOfDay: 0.1, // Time sensitivity
  ...
  async getRecommendations(driverId, params = {}) {
    try {
      const { limit = 20, minScore = 70, includeHazmat = false } = params;
      ...
      logger.info("ML: Recommendations generated", {
        ...
        avgScore: (
          recommendations.reduce((s, l) => s + l.mlScore, 0) / recommendations.length
        ).toFixed(1),
      });
  ...
  predictAcceptance(load, driverProfile) {
    ...
    if (load.miles <= driverProfile.avgAcceptedMiles * 1.1) adjustment += 0.1;
  ...
  isPreferredCorridor(load, driverProfile) {
    ...
    return driverProfile.preferredCorridors?.includes(`${load.pickupCity}-${load.dropoffCity}`);
  }

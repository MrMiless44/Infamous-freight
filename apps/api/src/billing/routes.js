...
router.post("/checkout", authenticate, async (req, res) => {
  try {
    ...
    if (!plan?.priceId) {
      return res.status(400).json({ ok: false, error: "Missing priceId for plan in env" });
    }

    const successUrl = env.STRIPE_SUCCESS_URL || "http://localhost:3000/account/billing?success=1";
    const cancelUrl = env.STRIPE_CANCEL_URL || "http://localhost:3000/pricing?canceled=1";
...
router.post("/portal", authenticate, async (req, res) => {
  try {
    ...
    const returnUrl = env.STRIPE_SUCCESS_URL || "http://localhost:3000/account/billing";
...
router.post("/webhook", async (req, res) => {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      await persist.setEntitlement(userId, "plan", String(session.metadata?.planKey || "paid"));
    ...
    res.status(500).json({ ok: false, error: err?.message || "webhook handler failed" });
  }
});

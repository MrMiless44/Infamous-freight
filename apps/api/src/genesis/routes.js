...
router.get("/profile", limiters.general, authenticate, requireScope("user:avatar"), (req, res) => {
  try {
    const userId = getUserId(req);
    const profile = resolveGenesisProfile(userId);
    res.json({ ok: true, profile, timestamp: new Date().toISOString() });
  } catch (err) {
    res.status(500).json({ ok: false, error: err?.message || "profile resolution failed" });
  }
});

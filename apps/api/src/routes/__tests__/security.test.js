...
describe("Security Middleware", () => {
  ...
  beforeEach(() => {
    ...
    // Simple test route
    app.get("/api/public", limiters.general, (_req, res) => {
      res.json({ ok: true });
    });

    app.get("/api/protected", limiters.general, authenticate, (_req, res) => {
  ...
  describe("requireScope middleware", () => {
    test("should allow request with required scope", async () => {
      ...
      const res = await request(app).post("/api/admin").set("Authorization", `Bearer ${token}`);
  ...
  describe("Rate Limiting", () => {
    ...
    test("should use keyGenerator to identify users", async () => {
      ...
      const res1 = await request(app).get("/api/protected").set("Authorization", `Bearer ${token}`);
  ...
  describe("Integration: Full auth chain", () => {
    test("should enforce full middleware chain", async () => {
      ...
      // Valid request
      const valid = await request(app).post("/api/admin").set("Authorization", `Bearer ${token}`);

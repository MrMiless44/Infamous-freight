...
const { handleValidationErrors, validateString, validateEmail, validateUUID } = require("../middleware/validation");
        ...
        try {
            ...
            if (!shipper) {
                return res.status(HTTP_STATUS.NOT_FOUND).json(
                    new ApiResponse({ success: false, error: "Shipper not found" })
                );
            }
...
router.get(
    ...
    async (req, res, next) => {
        try {
            ...
            if (!load) {
                return res.status(HTTP_STATUS.NOT_FOUND).json(
                    new ApiResponse({ success: false, error: "Load not found" })
                );
            }

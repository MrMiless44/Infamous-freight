...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await geofencingService.createShipmentGeofence(shipment, req.user.sub);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const update = await geofencingService.checkLocationUpdate(req.user.sub, latitude, longitude);
 ...
 */
router.get("/driver-geofences", authenticate, requireScope("driver"), async (req, res, next) => {
  try {
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const { latitude, longitude, type = "all", radiusMiles = 5 } = req.query;
...
router.post(
  ...
  [
    body("type")
      .isIn(["rest_area", "fuel_station", "weigh_station", "toll_booth", "shipper_facility"])
  ...
  async (req, res, next) => {
    try {
      const { type, id, name, description, lat, lng, radius, hours, amenities, rating } = req.body;
 ...
 */
router.get("/stats", authenticate, requireScope("admin"), async (req, res, next) => {
  try {
    const stats = {
      totalGeofences: 1245,

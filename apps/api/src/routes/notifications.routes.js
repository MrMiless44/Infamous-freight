...
router.post(
    ...
    async (req, res, next) => {
        try {
            ...
            const result = await pushNotificationService.sendToDriver(
                req.user.sub,
                testNotification
            );
...
router.put(
    ...
    authenticate,
    [
        body("notificationTopics")
            .optional()
            .isArray()
            .withMessage("Topics must be array"),
    ],
    handleValidationErrors,
    async (req, res, next) => {
        try {
...
router.post(
    ...
    [
        body("userIds")
            .isArray()
            .withMessage("Must provide array of user IDs"),
        body("templateType")
            .isString()
            .withMessage("Template type required"),
    ...
    async (req, res, next) => {
        try {
            const templates = Object.entries(
                pushNotificationService.templates
            ).map(([key, template]) => ({
                id: key,
...
router.post(
    ...
    [
        ...
        body("status")
            .isIn(["delivered", "read", "error", "not_sent"])
    ...
    async (req, res, next) => {
        try {
            ...
            await pushNotificationService.trackDelivery(
                notificationId,
                userId,
                status
            );

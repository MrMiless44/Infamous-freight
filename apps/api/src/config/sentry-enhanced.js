  ...
  try {
    Sentry.init({
      ...
      beforeSend(event, hint) {
        ...
        if ((event.status === 404 || event.fingerprint?.includes("404")) && !isDevelopment) {
          const url = event.request?.url || "";
          const safeNotFound =
            url.includes("favicon.ico") || url.includes("robots.txt") || url.includes(".map");
          if (safeNotFound) {
            return null;
          }
        }
        ...
        if (event.exception) {
          ...
          if (error instanceof Error) {
            ...
            if (!isDevelopment && error.message.includes("connection pool timeout")) {
              event.level = "warning";
              return event;
            }
          }
    ...
    logger.info(
      `Sentry initialized - environment: ${environment}, DSN: ${dsn.substring(0, 20)}...`,
    );
    return true;
  } catch (error) {

...
async function register(data: RegisterRequest): Promise<AuthTokens & User> {
  const response = await apiRequest<AuthTokens & User>(
    "/auth/register",
...
async function cancelSubscription(
  ...
  const response = await apiRequest<Subscription>(
    ...
    { method: "POST", body: { subscriptionId } },
  );
...
async function generateText(
  ...
  const response = await apiRequest<GeneratedText>(
    "/ai/generate",
    { method: "POST", body: data },
  );
...
async function analyzeSentiment(text: string): Promise<SentimentAnalysis> {
  const response = await apiRequest<SentimentAnalysis>(
...
async function generateEmbedding(text: string): Promise<Embedding> {
  const response = await apiRequest<Embedding>(
    "/ai/embedding",
    { method: "POST", body: { text } },
  );
...
async function processVoiceCommand(
  ...
  const response = await apiRequest<VoiceCommand>(
    "/ai/voice-command",

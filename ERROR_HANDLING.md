...
**Core Principle**: **All errors must be caught and delegated to the global error handler via `next(err)`, never thrown directly to the client.**
...
describe('Error Handling', () => {
  ...
  it('should include correlationId in error response', async () => {
    ...
    expect(response.body.error.correlationId).toMatch(
      /^err-\d+-[a-z0-9]+$/
    );
  });
});

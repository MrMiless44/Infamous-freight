...
test.describe("Payment Flow - Complete Checkout Journey", () => {
  ...
  test("should complete successful checkout for Pro tier", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="checkout-header"]')).toContainText(
      "Pro Plan - $99/month",
    );
  ...
  test("should validate form inputs", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="card-error"]')).toContainText("Invalid card number");
  });
  ...
  test("should support annual billing with discount", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="total-amount"]')).toContainText("$987"); // $99 * 12 * 0.83 (17% off)

    await expect(page.locator('[data-testid="savings-display"]')).toContainText("Save $201");
...
test.describe("Subscription Management", () => {
  test("should allow subscription upgrade", async ({ page, context }) => {
    ...
    await expect(page.locator('[data-testid="proration-explanation"]')).toContainText("prorated");
    ...
    await expect(page.locator('[data-testid="plan-price"]')).toContainText("$99");
  });
  ...
  test("should allow subscription downgrade", async ({ page, context }) => {
    ...
    await expect(page.locator('[data-testid="downgrade-warning"]')).toContainText("lose access");
    ...
    await expect(page.locator('[data-testid="downgrade-scheduled"]')).toContainText(
      "will change to Starter",
    );
  ...
  test("should handle subscription cancellation with retention", async ({ page, context }) => {
    ...
    await expect(page.locator('[data-testid="discount-offer"]')).toContainText("30% off");
    ...
    await expect(page.locator('[data-testid="cancellation-notice"]')).toContainText("active until");
  ...
  test("should allow subscription reactivation", async ({ page, context }) => {
    ...
    // Should succeed
    await expect(page.locator('[data-testid="reactivate-success"]')).toBeVisible({
      timeout: 10000,
    });
...
test.describe("Payment Method Management", () => {
  test("should allow adding new payment method", async ({ page, context }) => {
    ...
    // Should show success
    await expect(page.locator('[data-testid="card-added-success"]')).toBeVisible();

    // Should appear in payment methods list
    await expect(page.locator('[data-testid="payment-methods-list"]')).toContainText("•••• 4242");
  });
  ...
  test("should allow setting default payment method", async ({ page, context }) => {
    ...
    await expect(page.locator('[data-testid="default-badge-1"]')).toContainText("Default");
  });
  ...
  test("should allow removing payment method", async ({ page, context }) => {
    ...
    await expect(page.locator('[data-testid="payment-method-1"]')).not.toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe("Invoice Management", () => {
  ...
  test("should allow retrying failed payment", async ({ page, context }) => {
    ...
    if ((await failedInvoice.count()) > 0) {
      ...
      await expect(page.locator('[data-testid="retry-processing"]')).toBeVisible({ timeout: 5000 });

      await expect(page.locator('[data-testid="retry-result"]')).toBeVisible({ timeout: 15000 });
    }
  });
});

...
class TestRunner {
  ...
  async run() {
    ...
    const coverage =
      this.tests.length > 0 ? Math.round((this.passed / this.tests.length) * 100) : 0;
    console.log(
      `  ${coverage >= 90 ? colors.green : coverage >= 70 ? colors.yellow : colors.red}Coverage: ${coverage}%${colors.reset}\n`,
    );

...
- `/uploads` → Serves user-uploaded avatars from `api/public/uploads/`
- `/avatars/main` → Serves Phase 1 system defaults from `web/public/avatars/main/`
...
| File                        | Purpose                                |
| --------------------------- | -------------------------------------- |
| `api/src/avatars/routes.ts` | Express route handlers (6 endpoints)   |
| `api/src/avatars/store.ts`  | JSON persistence layer (atomic writes) |
| `api/src/auth/userId.ts`    | JWT token extraction utilities         |
| `api/src/config/env.ts`     | Environment validation with Zod        |
| `api/data/avatars.json`     | Store initialization file              |

### 4. **Directories Created**

```
api/public/uploads/         ← User avatar storage (per-user subdirs)
api/data/                   ← Store directory
api/src/avatars/            ← Avatar routes & store
api/src/auth/               ← Auth utilities
api/src/config/             ← Config & env validation
```
...
Copy 4 PNG files to `web/public/avatars/main/`:
...
The manifest at `web/public/avatars/main/manifest.json` already defines these.
...
```bash
git add api/src/avatars api/src/auth api/src/config api/public/uploads api/data web/public/avatars/main api/src/server.js
...
- **Phase 1:** Static files served from `web/public/avatars/main/` (no auth required)
- **Phase 2:** Dynamic uploads to `api/public/uploads/{userId}/` (authenticated users)
- **Persistence:** JSON store at `api/data/avatars.json` with atomic writes
...
- Integration guide: `api/src/avatars/MOUNT_SNIPPET.ts`
- Phase 1 manifest: `web/public/avatars/main/manifest.json`
- Server entry point: `api/src/server.js` (lines 113-118)
- Security middleware: `api/src/middleware/security.js` (authenticate, requireScope)
